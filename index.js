...
function processSum(numberList, callback) {
  return callback(
    numberList.reduce(function(accumulator, currentvalue) {
...
function getFullNames(runners) {
  let runnerNames = [];
  runners.forEach(function(runner){runnerNames.push(`${runner.last_name}, ${runner.first_name}`)});
  return runnerNames;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let result = runners.filter(runner => runner.shirt_size === tShirtSize);
  return result;
}
...
function tallyUpDonations(runners) {
  let donations = runners.reduce((accumulator, runner) => accumulator + runner.donation, 0);
  return donations;
}
